...
impl RemindersManager {
    fn get_all_reminders(&mut self) -> Vec<Reminder> {
        ...
        let time_output = Command::new("sh")
            .arg("-c")
            .arg("atq | cut -d ' ' -f 4 | cut -d ':' -f 1,2")
            .output()
            .expect("Failed to execute 'atq' command");

        let times = String::from_utf8(time_output.stdout)
            .expect("Invalid UTF-8 sequence")
            .lines()
            .map(|s| s.to_string())
            .collect::<Vec<String>>();

        // Get all reminder descriptions
        ...
        for id in &ids {
            ...
            let time_index = ids.iter().position(|x| x == id).unwrap();
            let time = times.get(time_index).unwrap().clone();
    ...
    fn new_reminder(&mut self, text: String, time: String) {
        Command::new("sh")
            .arg("-c")
            .arg(format!("echo 'notify-send \"{}\" -u normal -a Sidebar' | at {}", text, time))
            .output()
            .expect("Failed to execute 'at' command");
    }
    ...
    pub fn create_reminder_popup(&mut self, ctx: &egui::Context) {
        Window::new("Edit Task")
            ...
            .show(ctx, |ui| {
                ...
                ui.add(TextEdit::multiline(&mut self.reminder_text).min_size(Vec2::new(300.0, 100.0)));

                ui.label("Reminder Time:");
                ui.add(TextEdit::multiline(&mut self.reminder_time).min_size(Vec2::new(300.0, 50.0)));
    ...
    pub fn reminder_manager(&mut self, ui: &mut egui::Ui) {
        ...
        let container_rect = egui::Rect::from_min_size(Pos2::new(7.0, 405.0), Vec2::new(438.0, 170.0));
        ...
        ui.allocate_ui_at_rect(container_rect, |ui| {
            frame.show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .add(egui::Button::new("+").fill(Color32::from_rgb(255, 228, 225)).min_size(Vec2 { x: 2.0, y: 2.0 }))
                        .clicked()
                    {
		    ...
		    .id_source("reminder_scroll_area")
				    ...
				    ui.label(format!("  {} - {}", reminder.time, reminder.description));
