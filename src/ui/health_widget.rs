...
impl WaterManager {
    pub fn water_widget(&mut self, ui: &mut egui::Ui) -> Result<()> {
        ...
        let db: Db = self.open_db("$HOME/.local/share/SideBarWaterDb")?;
        ...
        let container_rect =
            egui::Rect::from_min_size(Pos2::new(7.0, 410.0), Vec2::new(410.0, 100.0));
            ...
            frame.show(ui, |ui| {
                ...
                if self.is_update || self.is_first_call {
                    match self.get_water_data(&db, date.clone()).unwrap() {
                        Some(amount) => {
                            ui.label(format!(" Date: {}", date.clone()));
                            ui.label(format!(" Water drunk: {} ml", amount));
                            ui.label(" You need to drink: 2000 ml");
                        }
                        None => {
                ...
                let buttons_rect =
                    egui::Rect::from_min_size(Pos2::new(198.0, 435.0), Vec2::new(35.0, 16.0));
                ui.allocate_ui_at_rect(buttons_rect, |ui| {
                    ui.horizontal(|ui| {
                            ...
                            .clicked()
                        {
                            ...
                            let _ = self.update_water_data(&db, date.clone(), self.water_amount);
                            self.is_update = true;
                        }
                        if ui
                            .add(
